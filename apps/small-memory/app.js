...
function applyEmbedUi() {
  ...
  if (els.saveJson) els.saveJson.style.display = 'none';
  if (els.loadJson) els.loadJson.style.display = 'none';
  if (els.exportTxt) els.exportTxt.style.display = 'none';
}
...
function pagesForRoomExport() {
  const pages = state.pages.filter(page => String(page.text || '').trim().length > 0);
  return pages.length ? pages : [getPage()];
}

function renderPagesToRoomCanvas(pages) {
  const targets = Array.isArray(pages) && pages.length ? pages : [getPage()];
  if (targets.length === 1) return renderPageToCanvas(targets[0]);
  ...
  grad.addColorStop(0, '#f8f7f3');
  grad.addColorStop(1, '#ece9df');
  ctx.fillStyle = grad;
  ...
  const gap = 42;
  const margin = 72;
  const cell = Math.floor(Math.min((size - margin * 2 - gap * (cols - 1)) / cols, (size - margin * 2 - gap * (rows - 1)) / rows));
  const totalW = cell * cols + gap * (cols - 1);
  const totalH = cell * rows + gap * (rows - 1);
  const startX = (size - totalW) / 2;
  const startY = (size - totalH) / 2;

  targets.slice(0, 9).forEach((page, index) => {
    const src = renderPageToCanvas(page);
    if (!src) return;
  ...
  if (targets.length > 9) {
    ...
    ctx.textAlign = 'right';
    ctx.fillText('+' + (targets.length - 9) + ' pages', size - 72, size - 54);
  }
...
function exportJPEG() {
  const targets = pagesForRoomExport();
  const roomCanvas = renderPagesToRoomCanvas(targets);
  const imageData = roomCanvas ? roomCanvas.toDataURL('image/jpeg', 0.94) : null;
  notifyMothershipImage(imageData, {
    title: 'SMALL MEMORY',
    note: (targets[0]?.text || '').split(/\n/).find(Boolean)?.slice(0, 80) || `${targets.length} pages`
  });
  if (EMBED_MODE) return;
...
window.__mothershipGetAsset = async function () {
  const targets = pagesForRoomExport();
  const canvas = renderPagesToRoomCanvas(targets);
  if (!canvas) return null;
  const imageData = canvas.toDataURL('image/jpeg', 0.94);
  const firstLine = String(targets[0]?.text || '').split(/\n/).find(Boolean) || `${targets.length} pages`;
  return { kind: 'image', imageData, title: 'SMALL MEMORY', note: firstLine };
};
