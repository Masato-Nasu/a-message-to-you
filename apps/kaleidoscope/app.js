...
function saveJPEG() {
  ...
  notifyMothershipImage(previewUrl, { title: '万華鏡', note: '36分割の万華鏡' });
  if (EMBED_MODE) return;
  if (canvas.toBlob) {
    canvas.toBlob((blob) => {
      if (!blob) {
        saveDataURL(name, previewUrl);
        return;
      }
...
function makeMothershipKaleidoscopeCanvas() {
  if (!canvas || !canvas.width || !canvas.height || !state.source) return null;
  draw();
  const size = 1536;
...
window.__mothershipGetAsset = async function () {
  const out = makeMothershipKaleidoscopeCanvas();
  if (!out) return null;
  const imageData = out.toDataURL('image/jpeg', 0.94);
  return { kind: 'image', imageData, title: '万華鏡', note: '視覚を反復させる' };
};
