export async function onRequestPost(context) {
  try {
    ...
    const embedBypass = request.headers.get('x-mothership-embed') === '1';
    if (!embedBypass && !isAuthenticated(request, env)) {
    ...
    const openaiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: env.OPENAI_MODEL_VISION || env.OPENAI_MODEL || 'gpt-4.1-mini',
        temperature: 0.2,
